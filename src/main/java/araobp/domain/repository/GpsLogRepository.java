...
public interface GpsLogRepository extends CrudRepository<GpsLog, Integer> {
	...
	public Iterable<GpsLog> getPreviousSession(@Param("current") Integer current);

	@Query("SELECT * FROM gps_log where session = (SELECT MIN(session) FROM gps_log WHERE session > :current)")
	public Iterable<GpsLog> getNextSession(@Param("current") Integer current);
